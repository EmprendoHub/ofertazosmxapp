...
export async function payPOSInstagramDrawer(data) {
  try {
    let {
      items,
      transactionNo,
...
export async function updateOneInstagramOrder(data) {
  try {
    let { transactionNo, paidOn, note, amount, orderId } =
      Object.fromEntries(data);
    let newOrderStatus;
...
export async function getAllPOSInstagramOrder(searchQuery) {
  try {
    await dbConnect();
    const session = await getServerSession(options);
    let orderQuery;
...
export async function getAllPOSInstagramProduct(searchQuery) {
  try {
    await dbConnect();
    // Find the product that contains the variation with the specified variation ID
    let productQuery = Product.find({
...
export async function getAllPOSInstagramProductNoFilter(searchQuery) {
  try {
    await dbConnect();
    // Find the product that contains the variation with the specified variation ID
    let productsData = await Product.find({
